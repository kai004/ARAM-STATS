...
async function create_summoner_entry(
    ...
    try {
      account_id = await utils.retry_async_function(
        league_api.get_account_id,
        account_args
      );
      summoner_puuid = await utils.retry_async_function(
        league_api.get_summoner_puuid,
        account_args
      );
    } catch (error) {
    ...
    try {
      ...
      last_processed_game_timestamp = await utils.retry_async_function(
        league_api.get_last_processed_game_timestamp,
        timestamp_args
      );
    } catch (error) {
    ...
    const recent_matches = await league_api.get_recent_matches(
      account_id,
      region,
      champ_dict,
      username
  ...
  async function get_full_matchlist(account_id, region, start_timestamp = 0) {
    ...
    do {
      ...
      matchlist = await utils.retry_async_function(
        league_api.get_subsection_matchlist,
        args
      );
...
async function multifetchMongoDataForLiveGame(
  ...
  let getLiveGame = async (username, region) => {
    ...
    try {
      players = await utils.retry_async_function(league_api.get_live_game, [
        sum_id,
      ]);
    } catch (error) {
